--[[*************************************************************************
     > File Name: header_filter_process.lua
     > Author: DongYu
...
do
    if v.enable then
        ...
        if v.config_name then
            ...
            if not v.module_dir or type(v.module_dir) ~= "string" then
        ...
        if v.module.header_filter then
            ...
            local handle = v.module.header_filter;
            if handle then
                if config then
                    conf = require(config);
                end
