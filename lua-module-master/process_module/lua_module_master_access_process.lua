...
ngx.log(ngx.DEBUG,"require module_conf:",ngx.var.module_conf);
local luafile = nil;
local config  = nil;
local conf    = nil;
...
do
    if v.enable then
        ...
        if v.module.access then
            --拼接执行文件
            luafile = v.module.filename;
            ngx.log(ngx.DEBUG,"luafile:", luafile);
            --require 方式进行执行代码
            local md = require(luafile);
            ngx.log(ngx.DEBUG,"require luafile:", luafile);
            if not md then
                ngx.log(ngx.ERR,"luafile is nil", luafile);
                ngx.exit(500);
            end
            local handle = v.module.access;
            if handle then
                if config then
                    conf = require(config);
                end
